...
interface TableParams<T> extends Options {
    idField: keyof T;
...
export const tableContext: TableView = new Proxy({} as TableView, {
    get(_target, prop) {
...
export function createTable<T>(node: HTMLElement, params: TableParams<T>) {
    ...
    function createTabulator(node: HTMLElement, orientation: Orientation) {
        ...
        _table.on('tableDestroyed', () => currentView = undefined);
    }
    ...
    function handleTableBuilt(table: Tabulator, useResponsiveLayout: boolean) {
        ...
        currentView = {
            ...
            tableDownloadTitle: params.tableDownloadTitle,
            tableMenuVisible: params.tableMenuVisible,
            table,
            useResponsiveLayout,
            groups: Object.keys(rowGroups),
            toggleGroup: (field: string) => toggleGroup(table, field),
        };
        params.onInit?.(currentView);
        return table;
    }
        ...
        params.columns
            ...
            .forEach((c) => {
                ...
                if (Array.isArray(c.headerMenu)) {
                    c.headerMenu.push({
                        label: `${get(t)('menu.table.group-by')} ${c.title}`,
